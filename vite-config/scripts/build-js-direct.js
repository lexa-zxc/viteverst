 ...
 */
async function processFile(filePath, visitedFiles = new Set(), processedFiles = new Map()) {
  ...
  for (const importMatch of imports) {
    ...
    if (importPath.startsWith('@utils/')) {
      resolvedPath = path.join(SRC_DIR, 'utils', importPath.replace('@utils/', ''));
    ...
    const importedContent = await processFile(resolvedPath, newVisited, processedFiles);
    
    // Заменяем импорт содержимым файла
    content = content.replace(fullImport, 
      `/* Инлайн импорт из ${path.relative(SRC_DIR, resolvedPath)} */\n${importedContent}\n`);
  }
...
async function build() {
  try {
    ...
    // Post-processing для удаления дублирующихся экспортов
    let finalContent = processedContent
      ...
      .replace(/\/\/ Константы проекта\s*const device_width[\s\S]*?const gsap_ease[\s\S]*?;/g, 
        function(match, offset) {
          // Сохраняем только первое определение
          return offset === processedContent.indexOf(match) ? match : '// (Определение констант уже было добавлено ранее)';
        });
    
    // Добавляем определения основных констант в начало файла, чтобы они точно были доступны
    finalContent = '// Основные константы проекта\nconst device_width = window.innerWidth;\nconst gsap_ease = "power4.out";\n\n' + finalContent;
    
    // Удаляем пустые блоки импортов и комментарии о повторных определениях
    finalContent = finalContent
      .replace(/\/\* Инлайн импорт из utils\\constants.js \*\/\s*\/\/ \(Определение констант уже было добавлено ранее\)\s*;/g, '')
      .replace(/\n\s*;\s*\n/g, '\n\n')
      .replace(/\n\n\n+/g, '\n\n');
    ...
    wrappedContent = header + wrappedContent;
    
    // Добавляем BOM-метку для корректного отображения UTF-8 в Windows
    const bomPrefix = '\ufeff';
    
    // Записываем результат
    fs.writeFileSync(OUTPUT_JS, bomPrefix + wrappedContent, { encoding: 'utf8' });
